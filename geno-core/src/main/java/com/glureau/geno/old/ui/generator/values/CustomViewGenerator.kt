    ...
    override fun generate(activity: Activity, name: String, data: Any?, root: ViewGroup): View {
        ...
        if (data != null) {
            for (property in Reflection.properties(kClass)) {
                ...
                val labelResId = activity.resources.getIdentifier("geno_" + property.name + "_label", "id", activity.packageName)
                ...
                val valueResId = activity.resources.getIdentifier("geno_" + property.name + "_value", "id", activity.packageName)
